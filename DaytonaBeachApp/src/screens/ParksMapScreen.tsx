...
export const ParksMapScreen: React.FC = () => {
  ...
  const initialRegion = hasGis
    ? {
        latitude: gisParks[0].latitude,
        longitude: gisParks[0].longitude,
        latitudeDelta: 0.05,
        longitudeDelta: 0.05,
      }
    : localParks.length
    ? {
        latitude: localParks[0].coordinates.latitude,
        longitude: localParks[0].coordinates.longitude,
        latitudeDelta: 0.05,
        longitudeDelta: 0.05,
      }
    : undefined;
    ...
    <View style={styles.container}>
      ...
      {!loading && !initialRegion && (
        <View style={styles.empty}>
          <Text style={styles.emptyText}>No parks available to display on the map.</Text>
        </View>
      )}
    </View>
  );
